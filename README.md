...
- **Behind a TLS-inspecting proxy?** If feed downloads or the SQL connection fail with
  `CERTIFICATE_VERIFY_FAILED`, point Python at your system CA bundle:
  `export SSL_CERT_FILE=/path/to/ca-bundle.pem` (and `REQUESTS_CA_BUNDLE` likewise). The Databricks
  SDK uses the system trust store, but `databricks-sql-connector` / `urllib` use certifi's.
